...
def draw_text_slow(surface, font, text, box, color):
    ...
    for word in words:
        ...
        draw_text(surface, font, text2, box, color)
        time.sleep(0.1)
        pygame.display.update()
...
while run:
    ...
    #detects when day changes
    if news != day:
        news = day
        #creates string for news
        start_string = "Day " + str(day) + " news: "
        numbers = list(range(8))
        # Shuffle the list in place, to get a random order of news
        random.shuffle(numbers)
        # Iterate over the shuffled list
        numbers = numbers[:-3]
        for i in numbers:
            #creates a bias for each stock
            ...
            start_string += text
            start_string += " "
        #creates a text box for the news 
        text_box = (280, 90, 700, 270)
        pygame.display.update()
        draw_text_slow(screen, font, start_string, text_box, BLACK)
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        if event.button == 1 and 650 <= x <= 760 and 20 <= y <= 60:
            day += 1
            pygame.draw.rect(screen, WHITE, (650, 20, 760, 60))
            y = HEIGHT//2
            x = 20
